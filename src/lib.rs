...
struct SpotRequestResult<'a> {
    ...
    setup_fns: HashMap<
        &'a str,
        &'a Box<Fn(&mut ssh::Session) -> Box<Future<Item = (), Error = failure::Error>>>,
    >,
...
impl TsunamiBuilder {
        ...
        FF: 'a + IntoFuture<Item = (), Error = Error>,
    {
        ...
        let all_the_things = futures::future::lazy(move || Self::configure_sg(log, ec2))
    ...
    ) -> Box<Future<Item = String, Error = Error> + 'a> {
    ...
    fn configure_ssh<'a>(
        log: &'a slog::Logger,
        ec2: &'a Box<rusoto_ec2::Ec2>,
    ) -> Box<futures::Future<Item = (String, String), Error = Error> + 'a> {
        // construct keypair for ssh access
    ...
    fn configure_placement_groups(
    ...
    ) -> Box<futures::Future<Item = Option<rusoto_ec2::SpotPlacement>, Error = Error>> {
        ...
        let future = ec2.create_placement_group(&req)
            .then(|r| r.context("failed to create new placement group"))
            .map(move |_| {
    ...
    fn issue_spot_requests<'a>(
        ...
        key_name: String,
        group_id: String,
    ) -> Box<Future<Item = SpotRequestResult<'a>, Error = Error> + 'a> {
        ...
        let all: Vec<_> = descriptors
            ...
            .map(|(name, &(ref setup, number))| {
                ...
                ec2.request_spot_instances(&req)
                    .then(move |r| {
                        r.context(format!("failed to request spot instances for {}", name))
                            .map(|v| (v, name))
                    })
                    .map(|(res, name)| {
                        (
        ...
        let future = futures::future::join_all(all)
            .map(move |res_v| {
    ...
    fn wait_for_instances<'a>(
        ...
        let wait_for_instances = futures::future::loop_fn(id_to_name, move |mut id_to_name| {
                ...
                .then(move |res| {
                    if let Err(e) = res {
                            ...
                            && msg.contains("does not exist")
                        {
                            ...
                            return Err(e).context(format!("failed to describe spot instances"));
                        }
                    }
        ...
        let future = if let Some(wait_limit) = max_wait {
            let future = tokio_timer::Deadline::new(
                ...
                match e.into_inner() {
                    ...
                    Some(e) => Either::B(futures::future::err(e)),
                }
            });
            Either::A(future)
        } else {
            ...
            .and_then(move |(id_to_name, instances)| {
                        ...
                        for reservation in reservations.reservations.unwrap_or_else(Vec::new) {
                            for instance in reservation.instances.unwrap_or_else(Vec::new) {
                                match instance {
                                    ...
                                    _ => {
                                        return futures::future::Loop::Continue(id_to_name);
                                    }
                                }
                            }
                        }
                ...
                }).map(|machine| (machine, term_instances))
                    .map_err(Error::from)
            });
            ...
            .flat_map(|(name, machines)| {
                ...
                machines.into_iter().map(move |machine| {
                        ...
                        .context("machine ip is not an ip address")
                    {
                        ...
                        Err(e) => Either::B(futures::future::err(Error::from(e))),
                    }.then(move |r| {
                        ...
                        if r.is_err() {
                            error!(log, "failed to ssh to {}:{}", name, &machine.public_dns);
                        }
